...
def login():
    if request.method == 'POST':
        ...
        cursor.execute("SELECT id, password, role FROM user1 WHERE name = %s", (username,))
...
def view_orders_to_place():
    cursor = mysql.connection.cursor()
    query = '''SELECT 
            e.name, e.equipment_id, d.name, d.contact_info, d.address
            FROM Equipment e
            LEFT JOIN Dealers d ON e.equipment_id = d.dealer_id
            WHERE e.threshold > e.quantity'''
    cursor.execute(query)
